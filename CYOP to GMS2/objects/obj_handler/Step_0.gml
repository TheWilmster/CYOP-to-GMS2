...
if (keyboard_check_pressed(ord("Z")) && !selected && room_name != "" && array_length(rooms) > 0) {
	switch (menu) {
		case "ctg":
			...
			selected = true;
			if (!file_exists("CTG Input/PizzaTower_GM2.yyp")) {
			...
			_yyp = file_text_open_write("CTG Export/PizzaTower_GM2.yyp");
			file_text_write_string(_yyp, compiled_yyp);
			_yyp = file_text_close(_yyp);
	
			if (!file_exists("CTG Input/PizzaTower_GM2.resource_order")) {
				var _resor = file_text_open_write("CTG Input/PizzaTower_GM2.resource_order");
				var struct = {
					FolderOrderSettings: [],
			...
			var _resor = file_text_open_read("CTG Input/PizzaTower_GM2.resource_order");
			_str = "";
			...
			_resor = file_text_open_write("CTG Export/PizzaTower_GM2.resource_order");
